# all in clash
使用mosdns + subconverter + clash + docker 进行路由转发实现全局透明代理。
...
1. docker创建网络,注意将网段改为你自己的
    ...
    `docker network create -d macvlan --subnet=192.168.88.0/24 --gateway=192.168.88.1 -o parent=eth0 _dMACvLan`
...
1. 请使用 http://clash.razord.top 管理内置规则
